...
namespace MediaBrowser.Plugins.VDR.Responses
{
    public class VLCResponse    
    {
        public VLCObj GetVLCResponse(Stream stream, IJsonSerializer json, ILogger logger)
        {
            ...
            {
                UtilsHelper.DebugInformation(logger,string.Format("[VDR] VLC Response: {0}", json.SerializeToString(root)));
                return root.JSONVlcObject.VLC_Obj;
            }
            logger.Error("[VDR] Failed to load the VLC from NEWA");
        ...
        {
            var root = json.DeserializeFromStream<RootObject>(stream);
            UtilsHelper.DebugInformation(logger,string.Format("[VDR] VLC Return: {0}", json.SerializeToString(root)));
